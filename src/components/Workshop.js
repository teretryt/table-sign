...
function Workshop() {
  ...
  const handleFileChange = (event) => {
    ...
    if (file && file.type === "image/svg+xml") {
      ...
      console.error("Lütfen geçerli bir SVG dosyası yükleyin.");
    }
  };
  ...
  useEffect(() => {
    ...
    let textColorSide = "#000000";
    let textEmissiveColor = "#000000";
    ...
    let fontData = null;
    let myText = null;//GÖRÜNECEK YAZI
    let thinText = null;// YAN YÜZ MAT OLURSA KULLANILACAK İNCE YAZI
    let background = null;// ARKAPLAN YANİ KASA
    ...
    const createScene = async () => {
      ...
      if (!guiRef.current) {
        ...
        // Text input
        textFolder.add({ text }, 'text').name("Metin").onChange((value) => {
          text = value;
          if (text.length > 0)
            reWriteText(scene);
        });
        ...
        textFolder.add({ backgroundIsVısıble }, 'backgroundIsVısıble').name("Arkaplan Görünümü").onChange((value) => {
          backgroundIsVısıble = value;
          reWriteText(scene);
        });
        // Yan yüzeyler aydınlatmalı mı?
        const ssiAttribute = textFolder.add({ sideFaceIsLit }, 'sideFaceIsLit').name("Yan Yüzeyler Aydınlatılsın mı?").onChange((value) => {
          if (sideFaceIsLit == value)
            return;
          sideFaceIsLit = value;
          if (frontFaceIsLit && sideFaceIsLit){
            sideFaceIsLit = false;
            ssiAttribute.setValue(false);
            ffiAttribute.setValue(false);
            singleColorBtn.domElement.click();
          }
          reWriteText(scene);
        });
        // Ön yüzey aydınlatılsın mı?
        const ffiAttribute = textFolder.add({ frontFaceIsLit }, 'frontFaceIsLit').name("Ön Yüzey Aydınlatılsın mı?").onChange((value) => {
          if (frontFaceIsLit == value)
            return;
          frontFaceIsLit = value;
          if (frontFaceIsLit && sideFaceIsLit){
            frontFaceIsLit = false;
            ssiAttribute.setValue(false);
            ffiAttribute.setValue(false);
            singleColorBtn.domElement.click();
          }
        ...
        //Aydınlat
        const singleLitBtn = textFolder.add({ singleLit }, 'singleLit').name("Aydınlat").onChange((value) => {
          if (singleLit == value)
            return;
          singleLit = value;
          reWriteText(scene);
        ...
        // Tek Renk Olsun mu?
        const singleColorBtn = textFolder.add({ isSingleColor }, 'isSingleColor').name("Tek Renk").onChange((value) => {
        ...
        // Colors Folder 
        const colorsFolder = guiRef.current.addFolder("Ön Yüzey Rengi");
        ...
        for (const [key, value] of Object.entries(textColorList)) {
          ...
          btn.domElement.classList.add("w-10", "h-10", "rounded-full", "m-1");
        ...
        const colorsFolder2 = guiRef.current.addFolder("Yan Yüzeyler Rengi");
        ...
        const mainColorFolder = guiRef.current.addFolder("Renk");
        ...
        // Case Radius Slider
        guiRef.current.add({ caseRadius }, 'caseRadius', 0, 50).name("Köşe Yarıçapı").onChange((value) => {
          caseRadius = value;
          reWriteText(scene);
        });
      }
    ...
    const reWriteText = (scene) => {
      ...
      textMaterial.diffuseColor = BABYLON.Color3.FromHexString(textColorSide);
      ...
      if (!(sideFaceIsLit && frontFaceIsLit) && !singleLit) {
        ...
        const thinTextMaterial = new BABYLON.StandardMaterial("thinTextMaterial", scene);
      ...
      if (backgroundIsVısıble){
        ...
        backgroundMaterial.diffuseColor = BABYLON.Color3.FromHexString(backgroundColor);

        let backgroundWidth = myText.getBoundingInfo().boundingBox.extendSize.x * 2 + padding * 2; // Sol ve sağ padding ekle
        let backgroundHeight = myText.getBoundingInfo().boundingBox.extendSize.y * 2 + padding * 2; // Taban yüksekliği
        ...
        background.position.y = (backgroundHeight - (padding * 2)) / 2; // Tabanı metnin arkasında konumlandır
        background.position.z = (backgroundDepth + 10) / 2; // Metnin önünde konumlandır
        background.material = backgroundMaterial;
      }
    };

    const writeSvg = (scene) => {
    };
  ...
  return (
    <div className="relative isolate my-10 px-6 pt-14 lg:px-8 h-screen">
      ...
      />
      <svg id="yourPathId" width="200px" height="200px" viewBox="0 -269.02099999999996 800 800" xmlns="http://www.w3.org/2000/svg">
          <g fill="#e61a27">
